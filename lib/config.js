...
export const appConfig = {
  ...
  // ── Cleanup model (non-destructive transcript proofreading only) ────────────
  // Separate from openAiTextModel so cleanup stays lightweight.
  openAiCleanupModel: process.env.OPENAI_CLEANUP_MODEL || "gpt-4o-mini",

  // ── Caption settings ────────────────────────────────────────────────────────
  enableCaptionChunking: process.env.ENABLE_CAPTION_CHUNKING !== "false",
  // Maximum number of words per caption segment (hard cap, 3-5 words for Reels/Shorts).
  maxWordsPerCaption: Number(process.env.MAX_WORDS_PER_CAPTION || 5),
  // Minimum time a caption stays visible (ms) — prevents flash-and-vanish.
  minCaptionDurationMs: Number(process.env.MIN_CAPTION_DURATION_MS || 1400),
  // Maximum time a single caption may linger (ms) — prevents frozen captions.
  maxCaptionDurationMs: Number(process.env.MAX_CAPTION_DURATION_MS || 3200),
