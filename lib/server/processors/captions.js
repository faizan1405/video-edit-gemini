...
// Minimum time a caption should stay visible so it doesn't flash and vanish.
const MIN_CAPTION_DISPLAY_SECONDS = 0.45;

// Small buffer added after the last word so the caption doesn't disappear the
// instant the speaker finishes the word.
const CAPTION_END_PADDING = 0.12;
...
export function buildCaptionSegments(mappedWords, language = "en") {
  ...
  const rawSegments = segments.map((words, index) => {
    ...
    // Enforce minimum display duration so captions never flash too briefly
    const end = Math.max(rawEnd, start + MIN_CAPTION_DISPLAY_SECONDS);
    return {
      id: `caption-${index}`,
      startSeconds: start,
      endSeconds: end,
  ...
  // Close small gaps between consecutive captions so there's no visible flicker.
  // If the next caption starts within 0.15s of the previous ending, extend the
  // previous caption to bridge the gap seamlessly.
  for (let i = 0; i < rawSegments.length - 1; i++) {
    const gapToNext = rawSegments[i + 1].startSeconds - rawSegments[i].endSeconds;
    if (gapToNext > 0 && gapToNext < 0.15) {
      rawSegments[i].endSeconds = rawSegments[i + 1].startSeconds;
    }
  }
