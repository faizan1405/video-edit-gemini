...
const LANGUAGE_PROMPTS = {
  ...
  hinglish:
    "Transcribe speech exactly as spoken. The audio may contain Hindi, Hinglish, and English mixed naturally. Preserve original wording, names, and accents. Do not translate or paraphrase.",
  ...
  auto: "Transcribe speech exactly as spoken. The audio may contain Hindi, Hinglish, or English. Preserve original wording, names, and accents. Do not translate or paraphrase."
};
...
export async function transcribeAudio(audioPath, captionLanguage = "auto") {
  ...
  // ── Align: gpt-4o-transcribe text + whisper-1 timestamps ──────────────────
  // Replace whisper's word tokens with the primary model's more accurate text.
  // This is the key fix for Hinglish caption accuracy: whisper tokens are often
  // garbled for Hindi/mixed speech, but gpt-4o-transcribe gets them right.
  const alignedWords = alignWords(bestTextResult.text, whisperResult.words);
  const alignedText = alignedWords.map((w) => w.token).join(" ");

  console.log(
    `[transcribe] Aligned ${alignedWords.length} words from ${primaryModel} text onto whisper-1 timestamps`
  );
